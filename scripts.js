$(document).ready(function () {
  ...
  $("#track-form").submit(function (e) {
    ...
    var newRow = `<tr>
                      ...
                      <td><button type="button" class="btn btn-danger btn-sm delete-btn"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
                    </tr>`;
  ...
  $('#tracks-table thead th').click(function() {
    ...
    $('.sort-button').not(sortButton).removeClass('sort-desc sort-asc').addClass('sort-inactive').html('&#x25BC;').data('order', 'desc');
    
    // Toggle the state of this button
    if(newOrder === 'asc') {
      sortButton.removeClass('sort-desc sort-inactive').addClass('sort-asc').html('&#x25B2;');
    } else {
      sortButton.removeClass('sort-asc sort-inactive').addClass('sort-desc').html('&#x25BC;');
    }
    sortButton.data('order', newOrder);

    rows.sort(function (a, b) {
      var A = $(a).children("td").eq(columnIndex).data("value") || $(a).children("td").eq(columnIndex).text().toUpperCase();
      var B = $(b).children("td").eq(columnIndex).data("value") || $(b).children("td").eq(columnIndex).text().toUpperCase();
  ...
  sampleTracks.forEach(function(track, index) {
      setTimeout(function() {
          var newRow = `<tr>
                            <td>${track.title}</td>
                            <td>${track.artist}</td>
                            <td data-value="${track.rating}">${track.rating}</td>
                            <td><button type="button" class="btn btn-danger btn-sm delete-btn"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
                        </tr>`;
          $("#tracks-table tbody").append(newRow);
